...
const ProjectTable: React.FC<ProjectTableProps> = ({ projects}) => {
  ...
  const handleRowClick = (project: ProjectProps, event: React.MouseEvent<HTMLTableRowElement, MouseEvent>) => {
    ...
    router.push(`/projects/details?_id=${project._id}&project_title=${encryptString(title)}`);
  };
  ...
  return (
    <div className="overflow-x-auto relative">
      <table className="table-auto min-w-full">
        <thead>
          <tr className="">
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">All</th>
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">Project Name</th>
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">Timeframe</th>
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">Milestone</th>
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">Status</th>
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">Team Member</th>
            <th className="px-4 py-2 text-[#68738D] font-inter text-base font-normal leading-5 text-left">Action</th>
          </tr>

        </thead>
        <tbody>
          {projects.map((project) => (
            <tr key={project._id} className="font-inter text-sm font-normal leading-5 text-left text-[#091228]" onClick={(event) => handleRowClick(project, event)}>
              <td className="border px-4 py-2 ">
                <input type="checkbox" id="checkboxInput" className="h-4 w-4 bg-blue-600" />
              </td>
              ...
              <td className="border px-4 py-2">
                <span
                  className={`px-2 py-1 rounded-[5px] text-[black] ${project.status === 'completed'
                    ? 'bg-[#C7C4FD]'
              ...
              <td className="border px-4 py-2 ">
                ...
                >
                  <p
                    className=" font-medium text-[10px] border border-grey-300 px-[5px] items-center">...</p>
                </button>
              </td>
            </tr>
          ...
          {isOptionsMenuOpen && (
            <div className="absolute top-0 right-[-1px] ml-[10px]">
              <div className="bg-white shadow-md rounded-md p-2">
                <button
                  type="button"
                  className="flex items-center  w-full text-left py-2 px-4 text-sm text-gray-700 hover:bg-gray-100"
                  onClick={() => handleCloseOptions()}
                >
                  <svg width="12" height="10" viewBox="0 0 12 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1.3335 5.66669L4.00016 8.33335L10.6668 1.66669" stroke="#091228" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                  Action A
                </button>
                <button
                  type="button"
                  className="flex items-center w-full text-left py-2 px-4 text-sm text-[#E2341D] hover:bg-gray-100"
                  onClick={() => handleCloseOptions()}
                >
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4 12L12 4M4 4L12 12" stroke="#F04438" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
